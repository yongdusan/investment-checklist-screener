...
export function scoreStock(stock, context = {}) {
  ...
  if (roic !== null || opMargin !== null || ocfToNetIncome !== null || roe !== null) {
    ...
    const hasTrendData = roicTrend3Y !== null || opMarginTrend3Y !== null;
    const trendsPositive =
      (roicTrend3Y === null || roicTrend3Y >= 0) &&
      (opMarginTrend3Y === null || opMarginTrend3Y >= 0);
    const solidCore = roic !== null && roic >= 12 && opMargin !== null && opMargin >= 8;
    const cashQualityOk = ocfToNetIncome === null || ocfToNetIncome >= 0.8;

    // Tier 1 (17pt): 핵심 지표 + 현금 질 + 3년 추세 모두 확인 (추세 데이터 없으면 불가)
    const strongQuality = solidCore && cashQualityOk && hasTrendData && trendsPositive;
    // Tier 2 (12pt): 핵심 지표 + 현금 질은 충족하나 추세 데이터 미확인 또는 부정적
    const goodQuality = solidCore && cashQualityOk;
    // Tier 3 (9pt): 부분적으로 수익성 조건 충족
    const mediumQuality =
      (roic !== null && roic >= 10) ||
    ...
    if (strongQuality) {
      ...
      breakdown.push(
        createBreakdownItem(
          ...
          17,
          true,
          `수익성의 질까지 확인되는 구간입니다. 3년 추세: ROIC ${fmt(roicTrend3Y)}%p, 영업이익률 ${fmt(opMarginTrend3Y)}%p`,
        ),
      );
    } else if (goodQuality) {
      rawScore += 12;
      reasons.push("ROIC와 마진은 양호하지만 3년 추세 확인이 더 필요합니다.");
      breakdown.push(
        createBreakdownItem(
          "profitability",
          12,
          true,
          `수익성 핵심 지표는 충족하나 3년 추세 데이터가 ${!hasTrendData ? "없어" : "일부 부정적이어서"} 추가 확인이 필요합니다.`,
        ),
      );
      ...
      breakdown.push(
        createBreakdownItem(
          ...
          9,
          true,
          `ROIC/현금전환율 또는 3년 추세 중 일부만 확인됩니다.${hasTrendData ? ` ROIC ${fmt(roicTrend3Y)}%p, 영업이익률 ${fmt(opMarginTrend3Y)}%p` : ""}`,
        ),
      );
    } else {
  ...
  if (interestCoverage !== null || debtRatio !== null || String(stock.netCash || "").trim()) {
    ...
    // 금융주는 구조적으로 부채비율이 높으므로 debtRatio 기준 제외, 이자보상배율/순현금만 적용
    if (netCash || (interestCoverage !== null && interestCoverage >= 5) || (!financialSector && debtRatio !== null && debtRatio <= 100)) {
      rawScore += 12;
      reasons.push("이자 감당 능력이나 순현금 측면에서 방어력이 있습니다.");
      breakdown.push(createBreakdownItem("balanceSheet", 12, true, financialSector ? "금융업 특성을 감안해 이자보상배율/순현금 기준으로 평가했습니다." : "재무 완충력이 우수한 편입니다."));
    } else if ((interestCoverage !== null && interestCoverage >= 2.5) || (!financialSector && debtRatio !== null && debtRatio <= 150)) {
      rawScore += 6;
      ...
      breakdown.push(createBreakdownItem("balanceSheet", 0, true, financialSector ? "금융업 기준으로도 이자보상배율이 낮은 편입니다." : "실질 상환 능력 기준으로는 부담이 있습니다."));
    }
  } else {
  ...
  // 데이터 완성도가 낮을수록 점수를 보수적으로 조정 (완성도 100% → 보정 없음, 60% → 약 84% 반영)
  const completenessRatio = maxScore > 0 ? availableMaxScore / maxScore : 0;
  const completenessMultiplier = 0.6 + 0.4 * completenessRatio;
  const normalizedScore = availableMaxScore
    ? Math.min(100, Math.round((rawScore / availableMaxScore) * completenessMultiplier * 100))
    : 0;
