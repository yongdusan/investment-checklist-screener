export const principles = [
  {
    ...
    rule: "업종 중위값 대비 할인율과 절대 PBR/PER를 함께 반영",
  },
  {
    key: "profitability",
    title: "수익성의 질이 유지되는가",
    ...
    rule: "ROIC, 영업이익률, 영업현금흐름/순이익 비율을 조합",
  },
  {
    key: "catalyst",
...
export function scoreStock(stock, context = {}) {
  ...
  if (roic !== null || opMargin !== null || ocfToNetIncome !== null || roe !== null) {
    ...
    if (strongQuality) {
      ...
      breakdown.push(createBreakdownItem("profitability", 17, true, "수익성의 질까지 확인되는 구간입니다."));
    } else if (mediumQuality) {
      rawScore += 9;
      reasons.push("수익성은 무난하지만 현금화 또는 자본효율 추가 검토가 필요합니다.");
      breakdown.push(createBreakdownItem("profitability", 9, true, "ROIC/현금전환율 중 일부만 확인됩니다."));
    } else {
