...
export function scoreStock(stock, context = {}) {
  ...
  if (per !== null || pbr !== null || (!financialSector && (evToEbitda !== null || fcfYield !== null))) {
    ...
    const discountParts = [
      ...
      !financialSector && evToEbitda !== null ? `EV/EBITDA ${evToEbitda}배` : null,
      !financialSector && fcfYield !== null ? `FCF Yield ${fcfYield}%` : null,
    ].filter(Boolean);
    ...
    if (strongQuality) {
      ...
      breakdown.push(
        createBreakdownItem(
          ...
          17,
          true,
          `수익성의 질까지 확인되는 구간입니다.${roicTrend3Y !== null || opMarginTrend3Y !== null ? ` 3년 추세: ROIC ${roicTrend3Y ?? "-"}%p, 영업이익률 ${opMarginTrend3Y ?? "-"}%p` : ""}`,
        ),
      );
      ...
      breakdown.push(
        createBreakdownItem(
          ...
          9,
          true,
          `ROIC/현금전환율 또는 3년 추세 중 일부만 확인됩니다.${roicTrend3Y !== null || opMarginTrend3Y !== null ? ` ROIC ${roicTrend3Y ?? "-"}%p, 영업이익률 ${opMarginTrend3Y ?? "-"}%p` : ""}`,
        ),
      );
    } else {
